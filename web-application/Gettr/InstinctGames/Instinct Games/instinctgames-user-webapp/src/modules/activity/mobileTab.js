...
function MobileTable(props) {
  const { table, tabId, typeIcon, getTabId, currencyConversion, moment } =
    props;
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex gap-1.5 font-Eurostile font-black text-ft2">
                      <img
                        ...
                        src={typeIcon(item.type).imgUrl}
                        alt={typeIcon(item.type).alt}
                      />
                      <p>{typeIcon(item.type).name}</p>
                    </div>
                ...
                <div className="grid place-items-center gap-2 mt-4 grid-cols-4 text-ft5 text-white">
                  <div>
                    ...
                    <p className=" font-EurostileMedium">
                      $
                      {(currencyConversion(item.currency) * item.price).toFixed(
                        2
                      )}
                    </p>
                  </div>
