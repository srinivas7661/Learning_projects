...
import TopNftTable from "./topNftsTable";
...
const Options = styled.option`
  ...
  opacity: 1;
`;
class TopNfts extends BaseComponent {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  async componentDidMount() {
    await this.nftTableData();
  }

  nftTableData = async () => {
    let [error, response] = await Utils.parseResponse(
      TotalSalesService.topNftsTable()
    );
    if (error) {
        ...
        <div className="display-flex justify-content-between">
          ...
          <div className="display-flex align-items-baseline">
            ...
            <Paper className="select-opt" elevation={0}>
              <Select
                onClick={()=>this.setState({changeIcon:!this.state.changeIcon})} 
              className={this.state.changeIcon?'select-arrow-up':'select-arrow-dn'}
