...
class Register extends BaseComponent {
  ...
  validateLoginForm = () => {
    this.setState({
      nameError: Utils.isEmpty(this.state.name)
        ? "Please enter name"
        : Utils.validateName(this.state.name)
        ? ""
        : "Invalid name",
    ...
    return (
      ...
      Utils.isPasswordValid(this.state.confirmPassword) &&
      this.state.password === this.state.confirmPassword
    );
  };
  ...
  onLoginClicked = (event) => {
    ...
    if (this.validateLoginForm()) Utils.basicAlert();
  };
