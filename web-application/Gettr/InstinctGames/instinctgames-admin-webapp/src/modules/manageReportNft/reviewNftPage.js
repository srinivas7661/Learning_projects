...
import GetReportedNftList from "../../services/contentService";
...
export default function ReviewNftPage(props) {
  const history = useHistory();
  // console.log(props.isOpen, "nft refportedsdsfdsdsfdsfsdfs")
  const [nftDetails, setNftDetails] = useState([]);
  const [reportedNftList, setReportedNftList] = useState([]);
  ...
  useEffect(() => {
    ...
    reportedList(props?.nftReportData?._id);
  }, []);
  ...
  const reportedList = async (contentId) => {
    const response = await GetReportedNftList.reportedNftDetailsList(contentId);
    if (response) {
      setReportedNftList(response);
      setLoading(false);
    }
  };
  ...
  const handleRemoveAndBlockUser = (id) => {
    if (id?.length > 0) {
      props.onClickDeleteNftAndClocUser(id);
    } else {
      CommonToasts.failureMessageSent("Please select atleast one user or Nft");
    }
  };
        ...
        <div>
          {loading === true ? (
            ...
            <ReviewPaper elevation={0}>
              <div className="display-flex flex-row gap-68 m-24 mb-column tab-column">
                <img
                  src={nftDetails?.cdnUrl || "/goku.jpg"}
                  width={271}
                  height={271}
                  className="br-4 mb-img-width"
                />
                <ColumnDiv>
                  <div className="display-flex flex-column w-284 w-per-100 w-251 w-352">
                    <p className="review-title">
                      {nftDetails?.nftType || "My Hero Academia"}
                    </p>
                    ...
                    <p className="description-short w-504 mb-description-review w-447 w-352">
                      {nftDetails?.description ||
                        "Shoto Todoroki is the tritagonist in the anime and manga franchise, My Hero Academia. He is a student at U.A. High School training to become a Pro Hero. He got into U.A."}
                    </p>
                    <div className="display-flex flex-row gap-20">
                      ...
                      <p className="owner-div">
                        <img
                          src={
                            nftDetails?.createdBy?.profileImage ||
                            "/defaultprofile.png"
                          }
                          className="img-div"
                        />
                  ...
                  <ButtonDiv>
                    <button
                      autoFocus
                      onClick={() => {
                        props.onClickDeleteNft();
                        // props.openReviewNft(false)
                      }}
                      className="review-remove-btn btn-width-height"
                    >
                      Remove NFT
                    </button>
                    <button
                      autoFocus
                      onClick={() => handleRemoveAndBlockUser(inputId)}
                      className="review-block-btn btn-width-height"
                    >
              ...
              >
                ...
                <TableBody>
                  {loading === true ? (
                    ...
                    reportedNftList &&
                    reportedNftList.length >= 1 &&
                    reportedNftList.map((row, index) => {
                      console.log(row._id, "row data");
                      return (
                        <StyledTableRow>
                          {/* <StyledTableCell className="w-20"></StyledTableCell> */}
                          <StyledTableCell
                            align="left"
                            className=" table-body w-25-per"
                          >
                            <img
                          ...
                          <StyledTableCell
                            align="left"
                            className="table-body w-10-per"
                          >
                            <img
                              className="delete-icon cursor-pointer"
                              onClick={() => props.onClickDeleteReport(row._id)}
                              src="/delete.svg"
                            />
                          </StyledTableCell>
                          <StyledTableCell
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={row._id === inputId}
                              name="report"
                              onChange={(e) =>
                                changeHandler(e.target.checked, row._id)
                              }
                            />
                          </StyledTableCell>
                        </StyledTableRow>
